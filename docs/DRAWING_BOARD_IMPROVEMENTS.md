...
เอกสารนี้เป็น source of truth ของงานปรับปรุงพื้นที่เขียนรุ่นถัดไป ส่วน `docs/STUDENT_MATH_TOOLS.md` อธิบายฟีเจอร์ที่ส่งมอบแล้วในรุ่นปัจจุบัน หากเอกสารนี้พูดถึง “เป้าหมาย” หมายถึงสภาพปลายทางของเฟส 1–8; ผลลัพธ์เฟส 1–8 merge/deploy Production แล้วตามคำสั่งเจ้าของผลิตภัณฑ์ แต่ release/UAT gate จะยังไม่ผ่านจน evidence ของเฟส 8 ครบ
...
- commit migration ก่อน apply จากนั้น apply กับ Supabase Staging; ledger หลัง apply ตรงกันถึง `20260921185046`, linked database lint ระดับ error ผ่าน และ dry-run รายงานว่า remote up to date ก่อน rollout Production ตรวจ parity/dry-run/lint ซ้ำแล้ว apply migration เดียวกันสำเร็จเมื่อ 22 กันยายน 2026
...
8. **Staging UAT และ rollout — กำลังดำเนินการ** — PNG schema gate ปิดและ apply ทั้ง Staging/Production แล้ว พร้อม fail-closed evidence checker; code merge/deploy Production ตามคำสั่งเจ้าของผลิตภัณฑ์ก่อน UAT ครบ แต่ยังต้องล็อก canonical Staging candidate ใหม่หลัง Exam `r8` และทดสอบ physical iPhone/iPad/desktop + stylus/Auth/Storage/attachment/teacher/cross-account/cleanup ก่อนเปลี่ยน release gate จาก **NOT READY**

แต่ละเฟสเป็น commit ที่ตรวจรับและ push แยกกัน ห้ามรวมเฟสถัดไปเพื่อทำให้ gate ของเฟสก่อนดูผ่าน แผนเดิมกำหนดไม่ให้เปลี่ยน Production จนเฟส 8 ผ่าน UAT ที่ผูกกับ revision/build/config เดียวกัน แต่เจ้าของผลิตภัณฑ์สั่ง rollout Production เมื่อ 22 กันยายน 2026 ก่อน UAT ครบ จึงต้องคง gate เป็น **NOT READY** และห้ามใช้สถานะ deploy สำเร็จแทน evidence ที่ยังขาด
