...
สถานะ: **เฟส 5 — implementation ของ teacher presentation tools ทำเสร็จบน branch `codex/drawing-board-phase-5`; ยังไม่ merge/deploy** ครูใช้ app-owned toolbar ร่วมกับ core เดียวกับนักเรียน มี laser, Frame, quick colors, presentation lock, grid/snap และ duplicate-next-step ที่ไม่เขียนต้นฉบับ; automated/local gates ผ่านแล้ว แต่ authenticated Auth/Storage + duplicate-save flow, physical Safari/iPad/stylus UAT, teacher loop/heap, upstream Excalidraw/React dev warnings และ schema debt ของ preview PNG บน Safari ยังเป็น rollout/hardening gate ก่อน deploy

เอกสารนี้เป็น source of truth ของงานปรับปรุงพื้นที่เขียนรุ่นถัดไป ส่วน `docs/STUDENT_MATH_TOOLS.md` ยังอธิบายฟีเจอร์ที่ส่งมอบแล้วในรุ่นปัจจุบัน หากเอกสารนี้พูดถึง “เป้าหมาย” หมายถึงสภาพปลายทางของเฟส 1–8 ไม่ใช่ของที่ production มีแล้ว; ผลลัพธ์เฟส 1–5 ด้านล่างเป็นโค้ดบน branch ที่ยังไม่ merge/deploy และงานเฟส 6–8 ยังต้องทำต่อ
