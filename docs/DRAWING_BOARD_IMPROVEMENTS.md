...
สถานะ: **เฟส 6 — implementation ของ teacher draft/save correctness ทำเสร็จบน branch `codex/drawing-board-phase-6`; ยังไม่ merge/deploy** draft ผูก exact `{questionId, slot, boardId}`, load/save เปลี่ยน identity แบบ atomic, การออกตรวจ draft ทุกกระดาน และ destructive action สร้าง one-step recovery ใน memory; automated/local gates ผ่านแล้ว แต่ authenticated saved-board load/save/delete/claim flow, physical Safari/iPad/stylus UAT, teacher loop/heap, accessibility/dev-runtime debt ที่บันทึกไว้, session-only Library และ schema debt ของ preview PNG บน Safari ยังเป็น rollout/hardening gate ก่อน deploy

เอกสารนี้เป็น source of truth ของงานปรับปรุงพื้นที่เขียนรุ่นถัดไป ส่วน `docs/STUDENT_MATH_TOOLS.md` ยังอธิบายฟีเจอร์ที่ส่งมอบแล้วในรุ่นปัจจุบัน หากเอกสารนี้พูดถึง “เป้าหมาย” หมายถึงสภาพปลายทางของเฟส 1–8 ไม่ใช่ของที่ production มีแล้ว; ผลลัพธ์เฟส 1–6 ด้านล่างเป็นโค้ดบน branch ที่ยังไม่ merge/deploy และงานเฟส 7–8 ยังต้องทำต่อ
