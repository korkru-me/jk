...
**อ่านแผนปัจจุบันก่อน:** ผู้ใช้เริ่ม N1 ของ [SEB_NO_SERVER_PLAN.md](SEB_NO_SERVER_PLAN.md) เพื่อประเมินระบบที่ไม่เพิ่ม SEB Server ส่วนร่างรหัสด้านล่างยังเป็นโค้ดเดิม และคำแนะนำ server/เฟส 4–5 เป็นประวัติแนวทางเดิม ไม่ใช่งานที่อนุมัติให้ดำเนินต่อ แผนใหม่มี native proof ตั้งแต่ N2 ก่อนต่อระบบเต็ม แต่ยังไม่เริ่มทดสอบจนผ่านข้อยุติ N1 และเตรียมทางออกกับผู้ใช้แล้ว
...
## ประวัติสิ่งที่ต้องตัดสินใจก่อนต่อเฟส 4 ของแผน SEB Server เดิม

หัวข้อนี้ใช้เฉพาะหากผู้ใช้กลับมาเลือกแนวทาง SEB Server อย่างชัดเจน ไม่ใช่เงื่อนไขว่ารหัสรายครูทุกวิธีต้องมี server เพิ่ม: ต้องทราบว่าจะใช้ SEB Server ที่มีอยู่แล้วหรือจัดเตรียมเครื่องใหม่ รวมถึงผู้ดูแล/สถานที่โฮสต์/ขอบเขต staging และ production ไม่มีการอนุมัติเช่าบริการ ลง Docker เปิด port หรือ deploy production ในงานนี้
...
## Checklist native ของแผน SEB Server เดิม — ไม่ใช้แทน N2/N7 ของแผนใหม่

ลำดับรอ implementation เฟส 4–5 และ server staging ด้านล่างเป็นแผนเดิม สำหรับแผนปัจจุบันให้ใช้ native proof N2 และตรวจรับ N7 ใน [แผนใหม่](SEB_NO_SERVER_PLAN.md) ไม่เลื่อนการพิสูจน์ต้นแบบไปไว้หลังทำ UI ทั้งหมด และไม่อ้างว่า Quit URL ปกติให้ signed server-authorized native exit ได้ รายการเดิมเก็บไว้เป็นประวัติของข้อกำหนดที่ต้องพิสูจน์หากกลับไปเลือก server protocol:
