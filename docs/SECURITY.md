...
- ห้ามส่ง `SEB_SESSION_SECRET`, CK, BEK หรือ Quit/Admin Password จาก server กลับไป client/log/submission; submission เก็บได้เฉพาะเวลา platform และ version ที่ผ่านตรวจ ส่วน raw request hash ไม่จำเป็นต่อ audit และห้ามเก็บ ข้อยกเว้นการจัดเก็บรหัสรายข้อสอบมีเฉพาะ **encrypted unpublished draft** ใน `exam_seb_password_drafts` ด้วย dedicated AES-GCM keyring และ lifecycle ที่กำหนดด้านล่าง ไม่ใช่ plaintext, native proof หรือรหัสกลาง และห้ามเก็บ secrets เหล่านี้ในตารางอื่น
...
- SEB draft page/action ตรวจ fresh auth + exact owning teacher + membership + online SEB exam ทุกครั้งก่อนใช้ admin client; RPC service-only ตรวจ owner ซ้ำและทำ atomic revision write + metadata audit ตาราง draft/events เปิด RLS และไม่มีสิทธิ์ browser อ่าน/เขียน ไม่รับ org/actor/owner context จาก browser และไม่ให้ co-teacher/global admin ข้าม owner policy
- รหัส draft รับจากฟอร์มผ่าน authenticated Server Action อยู่ใน request/server memory ระหว่าง validate/encrypt แต่ไม่เก็บ React state/localStorage/URL และไม่ส่งกลับใน response/log/exception envelope AES-GCM ผูก server-authorized org/teacher/assignment/revision; binding ไม่ได้แทน authorization ใช้ dedicated keyring เท่านั้น ห้าม reuse session/CK/BEK/Supabase keys จัดเก็บ key ใน deployment secret manager จำกัดสิทธิ์และ backup แยกจาก DB
- Draft secret หมดอายุ 30 วัน, job รายวันล้าง ciphertext และ metadata audit เกิน 90 วัน การ replace/discard ทิ้ง ciphertext เก่าเพราะยังไม่ใช่ published release; หากต่อไฟล์จริงต้องออกแบบ immutable releases แยก การล้าง production ไม่ได้ลบสำเนาใน DB backup หรือไฟล์ keyring ส่วนตัว ผู้ดูแลต้องมี backup retention/restore plan ด้วย
- Deployment ปิด draft feature โดย default และ missing migration/keyring ต้องแสดง unavailable ไม่คืน empty/success migration ใหม่ยังไม่ apply live การเลื่อน native test ไม่อนุญาตให้ promote draft เป็น applied หรือผ่อน CK + BEK gate ดู `docs/SEB_PHASE3.md` และ `docs/SEB_PASSWORD_ROLLOUT.md`
