...
6. หากตั้ง `random_question_count` ระบบสุ่ม subset จากคลัง `question_ids` ก่อน แล้วตรึง subset, ค่าตัวแปรสุ่ม เฉลย ลำดับข้อ และลำดับตัวเลือกไว้ใน `submission_answers` ของ attempt; reload/resume จึงไม่สุ่มใหม่
   - ถ้า `shared_random_seed` มีค่า `buildSkeletonBase()` ส่ง `rng` จาก `questionRandom(seed, question_id)` ให้ `randomizeVariables()` แทน `Math.random` ทุกคนจึงได้ค่าตัวแปรชุดเดียวกัน — ทางเดียวกันทั้ง `buildAssignmentAttempt()`, `buildAttemptQuestion()` (streak) และ `buildRetryAttempt()` · หน้าตัวอย่างมุมมองนักเรียนและโหมดสอนอ่าน `assignments.*` จึงเห็นตัวเลขชุดเดียวกับนักเรียน
...
6. โหมดสอนเป็น route ฝั่งครูแยกจาก teacher preview, ไม่สร้าง submission และโหลด Excalidraw ด้วย dynamic import; กระดานผูก assignment/question/creator/slot ผู้สร้างเก็บเฉลยได้ 3 ช่องต่อข้อ (Server Action บังคับตั้งแต่ 25 กันยายน 2026; CHECK ของฐานยังรับ 1–5 ให้กระดานเดิม) เปิด scene เดิมกลับมาแก้ได้ ครูร่วมอ่านของผู้อื่นแบบ read-only และไม่มี realtime broadcast บน branch drawing-board เฟส 1 scene ต้องผ่าน authorized Server Action กับ strict validator ก่อนเข้า browser รูปโจทย์เป็น trusted ingress ที่ server ตรวจ exact assignment/question/Storage source แล้ว rasterize, ตัด metadata และออก claim ส่วน legacy SVG ต้องผ่าน fail-closed XML/CSS boundary (decode เฉพาะ predefined/numeric references; ปฏิเสธ declaration/unknown reference, active content, style/xml-base, CSS escape และ external reference) ก่อนแปลงเป็น WebP และเข้า editor
