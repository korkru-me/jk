...
schema debt ที่สืบทอดจาก migration `20260904023417_math_work_png_preview.sql` ถูกปิดในเฟส 8 ด้วย `20260921185046_allow_png_math_work_preview_paths.sql`: CHECK ของ `student_work_artifacts`/`teaching_boards` และ student scope trigger รับ `.webp|.png` โดยคง namespace/path traversal/scope เดิม PGlite regression, Staging apply, migration parity, linked lint และ up-to-date dry-run ผ่านแล้ว; หลักฐานนี้ยังไม่แทน authenticated Safari attachment/teacher-board UAT และ migration ยังไม่ apply Production
