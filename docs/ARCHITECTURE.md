...
5. ถ้า `assignments.secure_browser_mode = 'seb_required'` เส้นทาง take จะออก challenge แบบ HMAC อายุ 5 นาที; `SebLaunchGate` อ่าน CK/BEK request hashes จาก SEB JavaScript API แล้ว Server Action ตรวจ exact origin/path/challenge, Config Key, Browser Exam Key และ version ก่อนออก HttpOnly session ที่ผูก user + assignment อายุ 12 ชั่วโมง ทุก server boundary ของ attempt ตรวจ session นี้ซ้ำ และบันทึกเฉพาะเวลา/platform/version ลง submission ไม่เก็บ raw key/hash ส่วน `/system-check` ออก challenge คนละ purpose หลังตรวจ roster เพื่อทดสอบเครื่องโดยไม่อ่านโจทย์/สร้าง attempt/เริ่ม timer และ server ปฏิเสธการ publish ข้อสอบ SEB เมื่อ production URL หรือ secret/CK/BEK ยังไม่พร้อม
   - รหัสออกที่ครูเจ้าของข้อสอบตั้งใช้ server-only core ตรวจสิทธิ์/ความแข็งแรงก่อน hash แล้ว Phase 2 ส่งเฉพาะ SHA-256 lower-case Base16 ไปยัง service-role-only RPC; RPC ล็อก exact assignment ตรวจ owner/org/eligibility/active attempt/revision ซ้ำและเพิ่ม immutable row ใน `assignment_seb_config_revisions` แบบ compare-and-swap โดยคืน metadata ที่ไม่มี hash เฟส 3 เพิ่มช่องตั้ง/เปลี่ยนรหัสใน flow สร้างและแก้ข้อสอบให้เฉพาะ exact owner, ล้าง plaintext ฝั่ง client หลัง action ทุกครั้ง, บังคับ published browser exam ที่เพิ่งเปิด SEB กลับเป็น draft และกัน publish ถ้ายังไม่มี revision โดยไม่เคยแสดงรหัส/hash เดิม การมี row นี้ยังไม่ใช่หลักฐานว่า `.seb`/CK/BEK พร้อม—artifact phase ต้องผูก exact revision ก่อนแจกนักเรียน
   - เมื่อ `system_check` ผ่าน Server Action ให้ service role เรียก `record_exam_seb_checkin()` เพื่อ upsert เฉพาะผลล่าสุดต่อ assignment + student หลัง RPC ตรวจซ้ำว่าข้อสอบ `published`, บังคับ SEB และนักเรียนยังอยู่ใน roster; เก็บเพียง `verified_at`, `valid_until` ไม่เกิน 12 ชั่วโมง, platform และ version ไม่เก็บผลที่ไม่ผ่านหรือ raw CK/BEK/hash, IP, user-agent หรือ fingerprint ครูที่จัดการข้อสอบอ่าน roster/check-in ผ่าน RLS ทุก 10 วินาทีและเมื่อกลับเข้าแท็บ จึงเห็นสถานะพร้อม/หมดอายุ/ยังไม่ตรวจโดยไม่สร้าง submission หรือใช้บันทึกนี้เป็น hard gate ตารางนี้ไม่ publish ผ่าน Postgres Changes เพราะ RLS ไม่ครอบคลุม payload ของ `DELETE`
...
6. หากตั้ง `random_question_count` ระบบสุ่ม subset จากคลัง `question_ids` ก่อน แล้วตรึง subset, ค่าตัวแปรสุ่ม เฉลย ลำดับข้อ และลำดับตัวเลือกไว้ใน `submission_answers` ของ attempt; reload/resume จึงไม่สุ่มใหม่
   - ถ้า `completion_rule = 'streak'` attempt ไม่มีความยาวให้ตรึง: `startSubmission()` เปิด attempt โดยไม่สร้างแถวคำตอบเลย แล้ว `drawNextStreakQuestion()` ต่อแถวทีละข้อผ่าน `buildAttemptQuestion()` ตัวเดียวกับ attempt ปกติ ทุกครั้งที่นักเรียนตรวจข้อที่ถืออยู่เสร็จ ตัวดึงข้อ idempotent — เจอข้อที่ยังไม่ตรวจก็คืนข้อเดิม ไม่ต่อแถวใหม่ — และอัปเดต `submissions.max_score` ทุกครั้งเพราะ `gradeAndFinalizeSubmission()` ไม่เคยคำนวณค่านี้ใหม่ · การตัดสินถูก/ผิดเกิดใน `checkAnswer()` ฝั่ง server แล้วเขียน `current_streak`/`best_streak`/`streak_reached`; ตรรกะการนับ การจบ และการเลือกข้อถัดไปอยู่ใน `lib/streak-run.ts` แบบ pure · ข้อใหม่ถึง browser ด้วย `router.refresh()` เพื่อให้ผ่าน `toSafeExamAnswer()` เสมอ ไม่เปิดทางที่สองให้ข้อมูลชุดเดียวกัน
...
9. หาก assignment เปิดคุมสอบ `useExamProctor()` ส่ง heartbeat/เหตุการณ์แบบ batch พร้อม opaque id ต่อแท็บไปยัง `recordProctorSignal()`; action ตรวจ session + exact attempt แล้วให้ service role เรียก RPC ที่ตรวจซ้ำและเขียน lease ใน `exam_proctor_connections` พร้อมสรุป/เหตุการณ์ใน `exam_proctor_sessions`/`exam_proctor_events` แบบ atomic หากมี lease สดเกินหนึ่งจะสร้าง `concurrent_connection` ฝั่งฐานข้อมูล สัญญาณที่ส่งไม่สำเร็จ retry แบบ backoff 1/3/10/30 วินาทีและยังมี heartbeat ทุก 15 วินาที ครูรับการเปลี่ยนแปลงผ่าน Supabase Realtime ภายใต้ RLS; เมื่อ channel หลุด dashboard อ่าน snapshot ผ่าน RLS ทุก 15 วินาทีและ reconcile ทุก 60 วินาทีแม้ channel ปกติ โดย replay event ที่มาระหว่าง query เพื่อไม่ย้อนสถานะ หน้าครู resolve เฉพาะชื่อของ student IDs ที่อยู่ใน roster/submission/event หลัง assignment ผ่าน RLS แล้ว จึงแสดงได้ว่าใครเกิดเหตุการณ์ใดโดยไม่เปิด profile อื่น ครูรับทราบ reviewable event ผ่าน service-role-only RPC แบบ first-ack-wins; metadata นี้อยู่บน event เดิมจึงกระจาย `UPDATE` ผ่าน Realtime และใช้ retention เดียวกัน โดย payload หลักมี trigger ห้ามแก้ ส่วน seen-ID registry รวมสัญญาณใหม่ต่อคนก่อน toast/เสียงและไม่เตือนซ้ำจาก snapshot หรือ acknowledgement update เสียงกับ Browser Notification ต้องเปิดจาก user gesture และทำงานเฉพาะขณะ dashboard ยังเปิด ไม่มี background push ส่วน `exam_watermark_enabled` แสดงชื่อกับรหัส attempt เฉพาะบน client เพื่อเป็นแรงเสียดทานต่อการส่งภาพ ไม่ได้บันทึกภาพหน้าจอ
   - `/assignments/[id]/proctor/report` เป็น read path หลังสอบบนตารางเดิม ไม่มี schema หรือ persisted report เพิ่ม หน้าอ่าน assignment ผ่าน session-bound client ก่อนเพื่อยืนยัน owner, co-teacher `admin/manage` หรือ super admin แล้วอ่าน event ผ่าน RLS อีกชั้น ตัวกรอง student, attempt, event kind และ acknowledgement ถูก parse ด้วย allowlist ฝั่ง server และ query แบบแบ่งหน้า; admin client ใช้หลัง authorization เท่านั้นเพื่อ resolve ชื่อจาก exact student IDs ของ retained proctor sessions ที่ RLS คืนให้ assignment นี้ (มี hard cap 2,000 attempts) จึงไม่ค้นหรือส่ง roster ทั้ง organization และไม่ส่งประวัติ event ทั้งชุดเข้า browser
   - ลำดับและเวลาหลักของรายงานมาจาก `exam_proctor_events.created_at` ซึ่งฐานข้อมูลกำหนด ส่วน `occurred_at_client` เป็นเวลาประกอบจากนาฬิกาที่ client ควบคุมและไม่ใช้เป็น authority รายงานใช้ถ้อยคำกลางว่าเป็นสัญญาณประกอบ ไม่คำนวณ risk score และไม่เปลี่ยนการรับทราบให้เป็นคำตัดสินนักเรียน
   - CSV ไม่สร้างจาก payload ของหน้าปัจจุบัน แต่ endpoint ตรวจสิทธิ์ผู้จัดการซ้ำแล้ว re-query ตามตัวกรองเดียวกัน ตรึงขอบเขต event ใหม่ด้วย ID สูงสุดจาก count query แรกและปฏิเสธเมื่อจำนวนที่อ่านไม่ตรงกัน กลไกนี้ไม่ใช่ transactional snapshot ของ acknowledgement ทุกช่อง จำกัด 10,000 แถว, 2,000 attempts และ 4 MiB; หากข้อมูลเปลี่ยนหรือเกินเพดานจะตอบ error โดยไม่ส่งไฟล์บางส่วนหรือเขียนว่าครบ ป้องกัน spreadsheet formula injection ในทุก cell ที่มาจากข้อมูลผู้ใช้ และส่ง `Cache-Control: private, no-store` กับ `X-Content-Type-Options: nosniff` ไม่มี migration ใหม่สำหรับเฟสนี้
...
10. แม่แบบ Excel สร้างผ่าน route handler แบบ `POST` หลังตรวจ session/สิทธิ์ และผูกแถวกับ template row token ที่ตรวจกลับในฐานข้อมูล การอัปโหลดอ่านไฟล์ในหน่วยความจำเท่านั้น แล้วเก็บเฉพาะ normalized preview/audit rows ไม่เก็บ binary ต้นฉบับ
11. การยืนยัน import ล็อก batch, ตรวจค่าคะแนนปัจจุบันเทียบกับ preview ทุกแถว แล้วเขียนทั้งชุดใน transaction เดียว; batch ที่ยืนยันแล้ว retry ได้โดยไม่เขียนซ้ำ
12. `/api/research/[id]/data-export` รับชนิดไฟล์ผ่าน strict bounded JSON, ตรวจ manage permission แล้วอ่าน participant/score แบบแบ่งหน้าจนครบภายใต้ RLS แบบไม่ระบุตัวตนจะไม่ query ชื่อหรือรหัสและสุ่มลำดับใหม่ก่อนกำหนดรหัสต่อไฟล์ ส่วนแบบมีชื่อ query profile เฉพาะเมื่อเลือก จากนั้นสร้าง `.xlsx` ในหน่วยความจำและให้ service-role-only RPC ตรวจสิทธิ์ actor ซ้ำก่อนบันทึก export metadata; ไม่มี binary หรือข้อมูลนักเรียนถูกเก็บใน audit/Storage

## เครื่องมือคณิตศาสตร์และพื้นที่เขียน

สเปกฟีเจอร์ที่ส่งมอบแล้วอยู่ใน `docs/STUDENT_MATH_TOOLS.md` ส่วนสเปกปรับปรุงพื้นที่เขียนรุ่นถัดไปอยู่ใน `docs/DRAWING_BOARD_IMPROVEMENTS.md` เฟส 1–6 ของ `STUDENT_MATH_TOOLS` เริ่มจาก schema/private Storage/RLS/Server Actions และ assignment settings แล้วตามด้วยแป้นคณิตศาสตร์, เครื่องคิดเลข, กระดาษทด local-only, artifact และโหมดสอน โดยแบ่ง runtime boundary ปัจจุบันไว้ดังนี้:
...
3. กระดาษทดที่ยังไม่แนบอยู่ใน IndexedDB แยกตาม user/submission/answer/part ด้วย versioned scene จำกัด 2 MiB/10,000 elements ไม่มี Server Action ต่อ stroke; purge ฉากหมดอายุ 7 วันเมื่อเปิด editor และล้างทั้ง submission หลังส่งสำเร็จแบบ best effort ส่วน preview ใช้ memory เท่านั้น บน branch drawing-board เฟส 3 record เดิมมี optional revision/fingerprint, exact attached artifact identity และ one-step recovery เพิ่มแบบ backward-compatible โดยไม่เปลี่ยน database/store/key/version; semantic fingerprint ไม่นับ tool/selection/pan/zoom และ client อ่าน metadata นี้ใหม่ก่อน submit เพื่อแยก attached current/stale/unverified
4. บน branch drawing-board เฟส 1 เมื่อผู้ใช้สั่งแนบ browser จึง render preview (WebP; PNG fallback บน Safari) และส่ง versioned scene ให้ Server Action หลัง action ตรวจสิทธิ์แล้วจะ validate และเขียน `scene.json` เอง จากนั้นคืนเฉพาะ signed token ของ preview กับ receipt อายุ 2 ชั่วโมง ฝั่งนักเรียน receipt ผูก actor + submission answer + part + source type + include-scene + upload id + preview format; ฝั่งครูผูก actor + assignment + question + slot + upload id + preview format Save action ตรวจ receipt, preview signature และ scene ที่ server เขียนไว้อีกครั้งก่อนบันทึก metadata การอัปโหลดไฟล์อย่างเดียวไม่ถือว่าแนบจน database reference สำเร็จ Migration `20260921185046_allow_png_math_work_preview_paths.sql` ทำให้ CHECK ของสองตารางและ student scope trigger รับ exact `preview.webp|preview.png` ตรงกับ bucket/server validation แล้ว; apply กับ Staging และ Production เมื่อ 22 กันยายน 2026 โดยไม่เปลี่ยน RLS/grant/ownership
...
6. โหมดสอนเป็น route ฝั่งครูแยกจาก teacher preview, ไม่สร้าง submission และโหลด Excalidraw ด้วย dynamic import; กระดานผูก assignment/question/creator/slot ผู้สร้างมี 5 slots เปิด scene เดิมกลับมาแก้ได้ ครูร่วมอ่านของผู้อื่นแบบ read-only และไม่มี realtime broadcast บน branch drawing-board เฟส 1 scene ต้องผ่าน authorized Server Action กับ strict validator ก่อนเข้า browser รูปโจทย์เป็น trusted ingress ที่ server ตรวจ exact assignment/question/Storage source แล้ว rasterize, ตัด metadata และออก claim ส่วน legacy SVG ต้องผ่าน fail-closed XML/CSS boundary (decode เฉพาะ predefined/numeric references; ปฏิเสธ declaration/unknown reference, active content, style/xml-base, CSS escape และ external reference) ก่อนแปลงเป็น WebP และเข้า editor
...
branch `codex/drawing-board-phase-1` วัดหลัง production build ได้ 17 initial chunks รวม 804,059 bytes raw / 246,601 bytes gzip (+33/+13 bytes จาก baseline) และ scan ไม่พบ Excalidraw, mathjs, `supabase-js`, `lib/supabase/client`, `sharp` package หรือ server-only board code ใน union

branch `codex/drawing-board-phase-2` วัดหลัง production build ได้ 17 initial chunks รวม 805,430 bytes raw / 247,011 bytes gzip (+1,371/+410 bytes จากเฟส 1 และยังต่ำกว่า gate 256,828 bytes gzip) และ scan ไม่พบ Excalidraw, mathjs, `supabase-js`, `lib/supabase/client`, `sharp` package หรือ server-only board code ใน union

branch `codex/drawing-board-phase-3` วัดหลัง production build ได้ 17 initial chunks รวม 808,768 bytes raw / 247,723 bytes gzip (+3,338/+712 bytes จากเฟส 2 และยังต่ำกว่า gate 256,828 bytes gzip) และ scan ไม่พบ Excalidraw, mathjs, `supabase-js`, `lib/supabase/client`, `sharp` package หรือ server-only board code ใน union

branch `codex/drawing-board-phase-4` วัดหลัง production build ได้ 17 initial chunks รวม 808,768 bytes raw / 247,726 bytes gzip (+0/+3 bytes จากเฟส 3 และยังต่ำกว่า gate 256,828 bytes gzip) และ scan ไม่พบ Excalidraw, mathjs, `supabase-js`, `lib/supabase/client`, `sharp` package, partial-eraser module หรือ server-only board code ใน union
...
branch `codex/drawing-board-phase-7` วัดหลัง production build ได้ 17 initial chunks รวม 809,105 bytes raw / 248,228 bytes gzip (-33/+246 bytes จากเฟส 6 และยังต่ำกว่า gate 256,828 bytes gzip) และ scan ไม่พบ Excalidraw, mathjs, `supabase-js`, `lib/supabase/client`, session Library, teacher editor หรือ server-only board code ใน union; `\\sharp` ยังเป็นคำสั่งสัญลักษณ์ของ KaTeX
...
บน branch `codex/drawing-board-phase-2` student host ใช้ `DrawingBoardController` เป็น typed command facade แทนการเรียก Excalidraw กระจายตาม toolbar; public API รับผิดชอบ tool/fit/scene boundary ส่วน adapter ภายใน `DrawingBoardCore` กัก action manager/history contract ของ Excalidraw 0.18.1 ไว้จุดเดียว ตรวจ nested shape แบบ fail closed และเปิดเฉพาะ native undo/redo, stroke/font selection actions กับ history subscription; toolbar จึงอ่าน common/mixed value จาก selection จริงและ `clearHistory()` resync ปุ่มเอง; input policy แยก `finger_draw`/`finger_pan` ออกจาก scene/persistence โดยเก็บ state ที่ `ExamClient`; touch pan ยืม Space โดยไม่เปลี่ยน active tool, two-finger gesture ปล่อยให้ editor จัดการ และ student host normalize technical `penMode=false` หลัง stylus เพื่อให้นิ้วทำตาม product toggle เสมอ; native student controls ยัง mount เพื่อ contract แต่ถูกซ่อนด้วย scope class และ app toolbar เป็น visual entry point เดียว; teacher composition ยังไม่เปลี่ยนในเฟสนี้; partial eraser, teacher presentation/save state และ hardening ยังทำในเฟส 4–7 รายละเอียด acceptance และ compatibility contract อยู่ใน `docs/DRAWING_BOARD_IMPROVEMENTS.md`

บน branch `codex/drawing-board-phase-3` pure scratchpad state layer สร้าง semantic fingerprint จาก visible elements, referenced files และ background แล้วแยก `editRevision`, `savedRevision` กับ attached artifact revision/fingerprint; IndexedDB persistence เขียน scene + metadata + one-step recovery ใน transaction/record เดียวก่อน destructive attached-scene load จึงไม่เปลี่ยน editor หาก read/download/validation/write ล้มเหลว Record รุ่นเก่าหรือ metadata ที่ตรวจเทียบไม่ได้ยังอ่าน scene ได้ แต่ artifact เป็น `attached_unverified` แทนการอ้างว่า current; `ExamClient` scan exact scope ตอนเปิดและก่อน submit เพื่อเตือน stale/unverified พร้อม logical question/part โดย server attachment requirement และ authorization เดิมไม่เปลี่ยน

บน branch `codex/drawing-board-phase-4` shared core เก็บทางลากยางลบเป็น scene coordinates แล้วให้ pure partial-eraser geometry ตัดเฉพาะ `freedraw` ด้วย analytic segment–capsule intersection; fragment คง style/pressure/group/frame/z-order และ normalized bounds ของเส้นหมุน ส่วน line/arrow/shape/text/image/frame ยังใช้ native whole-object eraser การเปลี่ยนฉาก custom ใช้ scheduled Store increment เดียวกับ native erase จึงทำ mixed gesture เป็น Undo/Redo หนึ่งขั้นและ deterministic; ก่อน apply ต้องผ่าน full scene validator รวมเพดาน 2 MiB/10,000 elements แบบ atomic โดยไม่เปลี่ยน envelope หรือ persistence contract
...
บน branch `codex/drawing-board-phase-7` teacher route เป็นเจ้าของ session Library เพิ่มอีก state หนึ่งที่แยกจาก draft และ persistence: item มีเฉพาะ vector/text ที่ sanitizer ตัด relation/link/custom data/file/app state แล้ว full-validate ด้วย student-safe policy, จำกัด 12 items/100 elements/256 KiB และ revalidate พร้อม re-id ก่อน insert จึงคงอยู่เมื่อ editor ของข้อหรือกระดาน remount แต่หมดอายุเมื่อ route owner unmount Compatibility manifest เป็น static JSON + test เท่านั้น ไม่ถูก bundle เข้า take route ส่วน `/exam-screen-lab/drawing-board` mount composition จริงหลัง local/Staging lab gate เพื่อใช้ browser/performance regression โดย Production ตอบ 404 Preview export import Excalidraw เฉพาะตอน user save จึงไม่ evaluate browser-only module บน server path

schema debt ที่สืบทอดจาก migration `20260904023417_math_work_png_preview.sql` ถูกปิดในเฟส 8 ด้วย `20260921185046_allow_png_math_work_preview_paths.sql`: CHECK ของ `student_work_artifacts`/`teaching_boards` และ student scope trigger รับ `.webp|.png` โดยคง namespace/path traversal/scope เดิม PGlite regression, Staging/Production apply, migration parity, linked lint และ up-to-date dry-run ผ่านแล้ว; `master`/Vercel Production rollout สำเร็จเมื่อ 22 กันยายน 2026 แต่หลักฐานนี้ยังไม่แทน authenticated Safari attachment/teacher-board UAT
...
- `select('*')` บน `questions` (5 จุด: print/preview ของงาน, แก้โจทย์, โจทย์กลุ่ม, เริ่มทำข้อสอบ) จะได้ `search_text` ติดมาด้วย เป็นข้อความที่ strip แล้วซ้ำกับ `question_text` ทั้งหมดอยู่ฝั่ง server ไม่ได้ถูกส่งต่อไป browser
