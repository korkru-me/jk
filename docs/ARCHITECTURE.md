...
4. บน branch drawing-board เฟส 1 เมื่อผู้ใช้สั่งแนบ browser จึง render preview (WebP; PNG fallback บน Safari) และส่ง versioned scene ให้ Server Action หลัง action ตรวจสิทธิ์แล้วจะ validate และเขียน `scene.json` เอง จากนั้นคืนเฉพาะ signed token ของ preview กับ receipt อายุ 2 ชั่วโมง ฝั่งนักเรียน receipt ผูก actor + submission answer + part + source type + include-scene + upload id + preview format; ฝั่งครูผูก actor + assignment + question + slot + upload id + preview format Save action ตรวจ receipt, preview signature และ scene ที่ server เขียนไว้อีกครั้งก่อนบันทึก metadata การอัปโหลดไฟล์อย่างเดียวไม่ถือว่าแนบจน database reference สำเร็จ อย่างไรก็ตาม tracked table CHECK และ student scope trigger ยังรับ path `preview.webp` เท่านั้น จึงต้องมี migration ใหม่ก่อน deploy PNG fallback จริง
