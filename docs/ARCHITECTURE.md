...
4. เมื่อผู้ใช้สั่งแนบ browser จึง render preview (WebP; PNG บน Safari ซึ่งเข้ารหัส WebP จาก canvas ไม่ได้) + versioned scene แล้วขอ signed upload token ที่ผูกกับ user/submission/answer/upload id จาก Server Action; action ตรวจไฟล์จริงซ้ำก่อนบันทึก metadata การอัปโหลดไฟล์อย่างเดียวไม่ถือว่าแนบจน database reference สำเร็จ
