...
**เอกสารต้นแบบในแผนเดิม:** แผนงานปัจจุบันคือ [SEB_NO_SERVER_PLAN.md](SEB_NO_SERVER_PLAN.md) N1–N8 ซึ่งยังไม่ผ่านจุดตัดสินใจ N1 ไม่ให้นับผลต้นแบบ `pswd` ในหน้านี้เป็นหลักฐานการเปิดแบบไม่ถามรหัส และผู้ใช้เป็นผู้เปิด native SEB เท่านั้น ข้อเสนอ SEB Server ด้านล่างเป็นประวัติ ไม่ใช่คำสั่งให้เดินต่อโดยอัตโนมัติ

งานต่อเนื่องที่ผู้ใช้อนุมัติ: [เฟส 2 — isolated SEB Server lab](SEB_PHASE2.md) เพิ่ม read-only connector และชุดรันทดลองเท่านั้น ไม่ได้ทำให้ native compatibility ของเฟส 1 ที่ยังค้างถือว่าผ่าน และยังไม่เปลี่ยน production gate
