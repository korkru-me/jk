...
เริ่ม physical-device UAT บน iPad จริงเมื่อ 21 กันยายน 2026 โดย layout แนวตั้ง/แนวนอน, navigator แบบ responsive และแป้นคณิตศาสตร์ที่ไม่เรียกคีย์บอร์ด iPad ซ้อนผ่าน แต่พบว่าหน่วยคำตอบถูกวางหลังปุ่มเปิดแป้น ทำให้ดูแยกจากช่องคำตอบ จึงแก้ component กลางให้ลำดับเป็น **ช่องคำตอบ → หน่วย → ปุ่มเปิดแป้น** ครบทั้งคำตอบเดี่ยว หลายข้อย่อย และช่องฝังในโจทย์ ผล runtime จำลอง 390×844, 768×1024, 1024×768 และ 1280×800 ไม่ล้นแนวนอน ค่าที่กดจากแป้นยังคงอยู่หลังปิด จากนั้น deploy และล็อก Staging candidate `r5` พร้อมตรวจ DOM จริงว่า `m/s²` และ `m` อยู่ระหว่างช่องกับปุ่มตามข้อกำหนด ผู้ใช้ยืนยันซ้ำบน iPad จริงว่าตำแหน่งถูกต้องและค่าคำตอบยังคงอยู่ จึงปิด regression จุดนี้ แต่ iPad suite ยัง pending จนกว่าจะทดสอบเมนู/overlay, touch gesture, กระดาษทด และกล้อง/ไฟล์ครบ ส่วน iPhone/Windows ผู้ใช้เลือกตรวจเองและจะแจ้งเมื่อพบปัญหา

รอบถัดมาผู้ใช้ยืนยันว่าเมนู **ดูทุกข้อ** และจับคู่แบบช่องผ่าน แต่ physical iPad บน candidate `r6` พบว่าการ์ดลากได้เพียงขวา→ซ้าย ซ้าย→ขวาไม่เริ่ม และการลากเฉียงจากฝั่งขวาบางครั้งกลายเป็นเลื่อนหน้า จึงทำให้การ์ดทั้งสองฝั่งเป็นพื้นที่ลากเต็มใบที่ browser ไม่แย่ง gesture และใช้เอฟเฟกต์กดค้างแบบเดียวกัน โดยเลื่อนหน้าจากพื้นที่ว่างนอกการ์ดแทน ผล local runtime ยืนยันซ้าย→ขวา, ขวา→ซ้าย, กดค้างทั้งสองฝั่ง และลากเฉียงจากฝั่งขวา พร้อม deploy/ล็อก candidate `r7` และตรวจ canonical Staging แล้ว เหลือทดสอบซ้ำบน iPad จริง

ผู้ใช้ทดสอบ candidate `r7` ซ้ำบน iPad จริงแล้วและยืนยันว่าจับคู่แบบลากเส้นผ่านครบทั้งสองทิศทาง การลากเฉียงไม่กลายเป็นเลื่อนหน้า และ feedback ทั้งสองฝั่งทำงานดี จึงไปทดสอบเรียงลำดับด้วยนิ้วต่อ โดย iPad suite ยังไม่ปิดจนกว่ารายการอุปกรณ์และ overlay ที่เหลือจะผ่าน

การกดค้างแล้วลากเรียงลำดับด้วยนิ้วบน iPad candidate `r7` ผ่าน การ์ดย้ายตำแหน่งและหน้าไม่เลื่อนแทน ขั้นถัดไปคือโหมดโฟกัส โดย iPad suite ยังคง `pending`

โหมดโฟกัสบน iPad candidate `r7` ผ่านทั้งเข้า/ออก การเลื่อนโดยไม่มีเนื้อหาหรือปุ่มถูกบัง และลำดับเดิมไม่ถูกรีเซ็ต ขั้นถัดไปคือกล่องยืนยันส่งคำตอบ

กล่องยืนยันส่งคำตอบบน iPad candidate `r7` ผ่านทั้งการแสดงผล การเลื่อนภายใน การยกเลิก และข้อมูลเดิมไม่หาย ขั้นถัดไปคือเครื่องคิดเลขวิทยาศาสตร์

เครื่องคิดเลขวิทยาศาสตร์บน iPad candidate `r7` ผ่านการคำนวณ `2 + 3 = 5`, การเลื่อนภายใน, การหมุนแนวตั้ง/แนวนอนโดยแผงไม่หลุดจอ และการปิดโดยคำตอบเดิมไม่หาย ขั้นถัดไปคือแป้นคณิตศาสตร์ของช่องคำตอบ

แป้นคณิตศาสตร์ของช่องคำตอบบน iPad candidate `r7` ผ่านแล้ว จากนั้นผู้ใช้ขอให้เครื่องคิดเลขแสดง comma คั่นหลักพัน จึงเพิ่ม formatter เฉพาะชั้นแสดงผลพร้อม mapping ตำแหน่งเคอร์เซอร์และการลบ โดยไม่เปลี่ยนนิพจน์ดิบหรือค่าที่ใส่กลับคำตอบ Local runtime และ canonical Staging ยืนยัน `1900052` → `1,900,052`, สูตร `1,900,052+1,000` → `1,901,052`, การวางเลขที่มี comma และ Backspace ตรงตัวคั่น พร้อมล็อก candidate ใหม่เป็น `r8`; ผู้ใช้ยืนยันผลบน iPad จริงว่าผ่านแล้ว ขั้นถัดไปคือกระดาษทดและ camera/file picker

กระดาษทดบน iPad candidate `r8` ผ่านทั้งการวาดต่อเนื่องด้วยนิ้วโดยหน้าไม่เลื่อนแทน, ยางลบ, undo/redo, สี/ขนาดเส้น, รูปทรง, ข้อความ, pinch zoom/pan, การหมุนจอ และการปิด–เปิดแผงโดยงานยังอยู่ การรีเฟรช `/exam-screen-lab` แล้ว scene หายเป็นพฤติกรรมที่ตั้งใจ เพราะ preview ปิด persistence; หน้าข้อสอบจริงเปิด local autosave และโหลด scene กลับตาม scope เดิม ขั้นถัดไปเหลือ camera/file picker
