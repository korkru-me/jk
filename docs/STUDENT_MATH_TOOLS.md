...
สเปกงานปรับปรุงกระดาษทด/กระดานสอนรุ่นถัดไป แผนเฟส 0–8, capability matrix, state machine และ baseline ล่าสุดอยู่ใน `docs/DRAWING_BOARD_IMPROVEMENTS.md` เป้าหมายในเอกสารนั้นยังไม่ใช่พฤติกรรม production จนกว่าแต่ละเฟสจะ implement และผ่าน gate ของตน
...
- รูปถ่ายจากกล้อง/เครื่องอยู่ใน `submission_answers.work_images` แยกจาก scene แก้เส้นภายในภาพไม่ได้และปัจจุบันไม่เปิดเป็นพื้นหลังของกระดาษทด; เปิดกลับมาแก้ได้เฉพาะ artifact ที่สร้างจากกระดาษทดและมี scene ต้นฉบับ
...
ช่องที่มีแป้นคณิตศาสตร์ครบไม่เรียก software keyboard ของ iOS/iPadOS ซ้อนอีก แต่ยังรับคีย์บอร์ดจริงบนคอมพิวเตอร์หรือคีย์บอร์ดที่จับคู่กับแท็บเล็ตได้ เมื่อแป้นเปิด เปลี่ยนเป็นชุดขั้นสูง หรือเครื่องหมุน ระบบเลื่อนช่องที่กำลังตอบให้อยู่เหนือ bottom sheet ตามความสูงจริง; บนจอสั้นแนวตั้งแผงสูงได้ไม่เกิน 70% และเลื่อนภายในแทนการบังทั้งจอ ส่วนมือถือแนวนอนที่สูงไม่เกิน 500px ใช้หัวแผงแบบแถวเดียวกับปุ่มหลัก 10 คอลัมน์ จึงยังคง touch target 44px โดยเหลือที่ให้มองโจทย์และช่องคำตอบมากขึ้น แถวคำตอบที่มีหน่วยต้องเรียงเป็น **ช่องคำตอบ → หน่วย → ปุ่มเปิดแป้น** เพื่อให้หน่วยสื่อความหมายกับค่าที่กรอกโดยตรง ไม่ถูกปุ่มเครื่องมือคั่นกลาง
...
- ไม่มี Server Action, Storage upload หรือ telemetry ต่อ stroke ในเฟสนี้ การกดแนบ/สร้าง WebP หรือ PNG preview/กลับมาแก้ artifact ที่แนบแล้วเป็นงานของเฟส 5
...
- rollout audit แบบอ่านอย่างเดียวยืนยันว่า bucket `math-work-artifacts` เป็น private จำกัด 5 MiB และรับ WebP/PNG/JSON (`20260904023417` เพิ่ม bucket MIME); drawing-board เฟส 8 เพิ่ม `20260921185046` ให้ CHECK ของ artifact/teaching board และ student scope trigger รับ exact `.webp|.png` ตรงกันและ apply ทั้ง Staging/Production เมื่อ 22 กันยายน 2026 ตาราง artifact/teaching board ไม่มีข้อมูล QA ค้าง และ production cleanup dry-run รอบเดิมจบโดยไม่ลบข้อมูล แต่ authenticated Safari flow ยังรอ UAT แยกและการ deploy Production ไม่นับแทนหลักฐานนี้
