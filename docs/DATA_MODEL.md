...
- `shared_random_seed` สร้างฝั่ง server เท่านั้น (`createSharedRandomSeed`, 1 ถึง 2^31-1, CHECK `> 0`) browser ส่งมาแค่ว่าเปิดหรือปิด · เปลี่ยนเปิด/ปิดไม่ได้หลังมี submission แรก และการบันทึกหน้าแก้ไขขณะยังเปิดอยู่ไม่สร้าง seed ใหม่ · ตอนเริ่ม attempt ค่าตัวแปรของแต่ละข้อสุ่มจาก generator ที่ตั้งต้นด้วย seed + `question_id` (`lib/math/shared-random.ts`) แล้วตรึงลง `submission_answers.random_values` เหมือนเดิม การตรวจคำตอบจึงไม่อ่านคอลัมน์นี้ · ทำสำเนางานคัดลอก seed เดิมไปด้วย
...
- **`questions.solution_text` อาจมี `<img>`** (ตั้งแต่ 25 กันยายน 2026) — รูปที่ครูแทรกในช่องพิมพ์เฉลยเป็นไฟล์ `solution-inline_*` ใน `question-images` พร้อม `width`/`height` ที่ครูลากไว้ และเป็นของข้อความ ไม่อยู่ใน `solution_image_urls` · orphan sweep ครอบคลุมเพราะจับคู่กับทั้งแถว แต่รูปที่ลบออกจากข้อความไม่ถูกลบจาก Storage ทันที ต้องรอการกวาดใน ตั้งค่า
