...
function extraDataFor(
  ...
  // SAMPLE_QUESTIONS holds one or more samples per type — จับคู่ carries both of
  // its answering layouts. The lab renders the first, which is each type's
  // canonical example.
  const props = SAMPLE_QUESTIONS[source][0].props
...
export function buildExamScreenQaQuestions(): Question[] {
  return EXAM_SCREEN_QA_CASES.map((testCase, index) => {
    const sample = SAMPLE_QUESTIONS[testCase.source][0]
