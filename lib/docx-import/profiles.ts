...
/** Read off a real worksheet of rotational-motion problems, not invented. */
const ANSWER_BRACKET: ImportRule = {
  id: 'answer-bracket',
  text: 'เฉลยใส่ในวงเล็บท้ายข้อ เช่น (2.5) · (25 rad/s) · (14pi) — ข้อย่อยที่มีเฉลยของตัวเอง ใส่ท้ายข้อย่อยนั้น',
}
...
export const PROFILE_BY_TYPE: Record<QuestionType, ImportProfile> = {
  ...
  written: {
    ...
    rules: [
      ...
      ANSWER_BRACKET,
      { id: 'answer-multi', text: 'ข้อที่ต้องตอบหลายค่า คั่นด้วยจุลภาคในวงเล็บเดียว เช่น (200 m/s, 10 m/s²) ระบบแยกเป็นช่องกรอกให้ช่องละคำตอบ' },
      EQUATION,
    ],
    sample: [
      line('heading', t('ใบงานที่ 3 แรงและกฎการเคลื่อนที่')),
      line('question', t('วัตถุมวล 5 กิโลกรัม ถูกแรง 20 นิวตันกระทำในแนวราบ')),
      line('part', t('จงหาความเร่งของวัตถุ (4 m/s²)')),
      line('part', t('ถ้าวัตถุเริ่มจากหยุดนิ่ง หลังจาก 4 วินาที วัตถุมีความเร็วเท่าใด (16 m/s)')),
      line('question', t('ล้อหมุนด้วยความถี่คงที่ 420 รอบต่อนาที จงหาอัตราเร็วเชิงมุมในหน่วยเรเดียนต่อวินาที (14pi)')),
      line('question', t('พัดลมช้าลงจาก 750 เป็น 150 รอบต่อนาที ใน 30 วินาที จงหาความเร่งเชิงมุม และจำนวนรอบที่หมุนได้ (-2pi/3, 225 รอบ)')),
    ],
    limits: [
      'ค่าคลาดเคลื่อนที่ยอมรับตั้งให้เป็น 0.1 ทุกข้อ ปรับรายข้อได้ในฟอร์ม',
      'ข้อที่ไม่มีวงเล็บเฉลยจะเข้ามาเป็นบรรยายให้ครูตรวจเอง ไม่ถูกทิ้ง',
  ...
  fill_blank: {
    ...
    blurb: 'ข้อความที่เว้นช่องให้นักเรียนเติมคำหรือวลี',
    status: 'planned',
    rules: [
      NUMBERING,
      { id: 'blank-answer', text: 'พิมพ์คำตอบไว้ตรงตำแหน่งที่จะเป็นช่องว่าง แล้วทำเครื่องหมายด้วยตัวอักษรสีแดง — ระบบจะเปลี่ยนคำนั้นเป็นช่องให้นักเรียนกรอก' },
      { id: 'blank-count', text: 'หนึ่งข้อมีช่องว่างได้หลายช่อง ทำเครื่องหมายทุกคำที่เป็นคำตอบ' },
    ],
    sample: [
      line('question', t('หน่วยของแรงในระบบเอสไอคือ '), key('นิวตัน')),
      line('question', t('น้ำเปลี่ยนสถานะเป็นไอที่อุณหภูมิ '), key('100'), t(' องศาเซลเซียส')),
    ],
    limits: ['ช่องที่รับคำตอบได้หลายแบบ (เช่น สะกดได้สองอย่าง) ต้องเพิ่มคำที่ยอมรับเองในเว็บ'],
...
export const AUTO_PROFILE: ImportProfile = {
  ...
  sample: [
    ...
    line('question', t('ลวดความต้านทาน 20 โอห์ม ต่อกับความต่างศักย์ 12 โวลต์ จงหากระแสไฟฟ้าที่ไหลผ่าน (0.6 A)')),
  ...
  limits: [
    ...
    'ข้อที่ไม่มีทั้งตัวเลือกและวงเล็บเฉลย จะเข้ามาเป็นอัตนัยให้ครูตรวจเอง',
    EMF_LIMIT,
  ],
