...
export interface Question {
  ...
  /** Lowercased, markup-stripped title + body — what the คลัง searches, so that a
   *  query matches the words a teacher reads rather than the HTML around them.
   *  Generated by the database; never write to it. */
  search_text: string
  /** How many tags this question carries, so the คลัง can order by it —
   *  PostgREST orders by columns, not by `array_length(tags, 1)`.
   *  Generated by the database; never write to it. */
  tag_count: number
  created_at: string
  updated_at: string
